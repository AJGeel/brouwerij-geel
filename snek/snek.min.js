...
var canvas=document.getElementById("game-area"),width,height,tileSize,scale,fps,ctx,snake,score,isPaused,step,gameOver=!1,lWidth,inputPending,fontSize,fontFamily,gold,darkGold,lightGold,chomps_1,chomps_2,chomps_3,chomps_4,burp,boom,hiHat,snare,kick;
function init(){scale=2;tileSize=20*scale;fps=13;score=0;isPaused=!1;lWidth=1;step=0;fontSize=30*scale;fontFamily="Sen";gold="hsla(38 40% 60% / 1)";darkGold="#423724";lightGold="hsl(38deg 100% 90%)";chomps_1=new sound("s_chomps_1.mp3",1);chomps_2=new sound("s_chomps_2.mp3",1);chomps_3=new sound("s_chomps_3.mp3",1);chomps_4=new sound("s_chomps_4.mp3",1);burp=new sound("s_burp.mp3",.7);boom=new sound("s_boom.mp3",.7);hiHat=new sound("s_hiHat.mp3",.1);kick=new sound("s_kick.mp3",.7);snare=new sound("s_snare.mp3",
.7);var a=document.querySelectorAll(".angel-di-maria")[0];gameOver||(width=scale*tileSize*Math.floor((a.offsetWidth-128)/tileSize),height=scale*tileSize*Math.floor(a.offsetHeight/tileSize),canvas.width=width,canvas.height=height);ctx=canvas.getContext("2d");snake=new Snake({x:tileSize*Math.floor(width/(2*tileSize)),y:tileSize*Math.floor(height/(2*tileSize))},""+gold);food=new Food(spawnLocation(),""+lightGold);gameOver=!1}function game(){canvas.className="";init();update()}
